...
def evaluate_model(
    ...
    """
    Evaluate a trained model or pipeline on test data producing metrics.
    
    Args:
        model_or_pipeline: Trained sklearn model or Pipeline (with preprocessing).
        X_test: Raw features for test dataset (pipeline will preprocess if needed).
        y_test: True labels/values for test dataset.
        task_type: 'classification' or 'regression'. If None, inferred.
        plot: Whether to show/save evaluation plots (currently disabled for API).
    ...
    # Perform predictions (pipeline handles preprocessing automatically)
    try:
    ...
    if task_type == "classification":
        ...
        if hasattr(model_or_pipeline, "predict_proba") and len(np.unique(y_test)) == 2:
            try:
                probs = model_or_pipeline.predict_proba(X_test)[:, 1]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print(
            "Usage: python tester.py <model_path> <X_test.csv> <y_test.csv>\n"
